...
namespace System.Windows.Forms
{
    ...
    public sealed partial class Application
    {
        ...
        {
            ThreadContext ctx = ThreadContext.FromId(User32.GetWindowThreadProcessId(handle.Handle, out _));
            Debug.Assert(
                ctx is not null,
        ...
        internal static void ParkHandle(HandleRef handle, IntPtr dpiAwarenessContext)
        {
            ...
            ThreadContext ctx = GetContextForHandle(handle);
            if (ctx is not null)
            {
                ctx.GetParkingWindow(dpiAwarenessContext).ParkHandle(handle);
            }
        }
        ...
        {
            ThreadContext ctx = ThreadContext.FromCurrent();
            if (ctx is not null)
            {
                cp.Parent = ctx.GetParkingWindow(dpiAwarenessContext).Handle;
            }
        }
        ...
        internal static void UnparkHandle(HandleRef handle, IntPtr context)
        {
            ...
            if (ctx is not null)
            {
                ctx.GetParkingWindow(context).UnparkHandle(handle);
            }
        }
